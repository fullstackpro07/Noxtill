...
describe('Core journey (e2e)', () => {
  ...
  it('signup: creates a real business + owner + tokens', async () => {
    ...
    expect(signupBody.business.id).toBeDefined();
    accessToken = signupBody.accessToken;
    businessId = signupBody.business.id;
  });
  ...
  it('onboarding: creates a real sellable product', async () => {
    ...
    const productBody = res.body as { id: string };
    expect(productBody.id).toBeDefined();
  ...
  it('first sale: completes a real sale, creating an order and a review request', async () => {
    ...
    const saleBody = res.body as { id: string; status: string };
  ...
  it("nightly close: today's real sale appears in the day aggregate", async () => {
    ...
    const dayBody = res.body as { ordersCount: number; revenue: number };
    expect(dayBody.ordersCount).toBeGreaterThanOrEqual(1);
    expect(dayBody.revenue).toBeGreaterThanOrEqual(25);
  });
