...
} from '../notifications/notification-preferences.constants';
import {
  CategoryDef,
  GroupDef,
  HubCtx,
...
export class SettingsHubService {
  ...
  async ctxFor(user: AuthenticatedUser): Promise<HubCtx> {
    if (
      ...
      throw new ForbiddenException(
        'Settings are available to owners and managers.',
      );
    }
    const businessId =
      this.cls.get<string>(CLS_KEY_BUSINESS_ID) ?? user.businessId;
    const caps =
  ...
  private async rowsOf(group: GroupDef, ctx: HubCtx): Promise<RowDef[]> {
    return [
      ...(group.rows ?? []),
      ...(group.dynamicRows ? await group.dynamicRows(ctx) : []),
    ];
  }
    ...
    const locked =
      def.write && !editable
        ? def.requires === 'owner'
          ...
          : 'Your role does not have permission to change this.'
        : null;
    return {
      key: def.key,
      label: def.label,
      ...
      control: def.write
        ? (state.control ?? null)
        : state.control && state.control.type === 'action'
          ? state.control
          : null,
      editable,
      locked,
  ...
  async detail(categoryKey: string, ctx: HubCtx) {
    const cat = this.category(categoryKey);
    const pins = await this.prisma.settingPin.findMany({
      where: { businessId: ctx.businessId, userId: ctx.userId, pinned: true },
    });
        ...
        const rows = await Promise.all(
          defs.map(async (def) => {
            try {
              ...
              this.logger.warn(
                `Setting ${categoryKey}.${def.key} could not be read: ${(error as Error).message}`,
              );
              return this.toRow(
                def,
                { value: 'Unavailable', tone: 'amber' },
                ctx,
                pinned,
                categoryKey,
              );
    ...
    const health =
      categoryKey === 'home'
        ? allHealth
        : allHealth.filter((i) => i.category === categoryKey);
    ...
    return {
      ...
      notice: cat.notice
        ? {
            text: cat.notice.text,
            icon: cat.notice.icon,
            action: cat.notice.action ?? null,
          }
        : null,
  ...
  private normalize(def: RowDef, state: RowState, value: HubValue): HubValue {
    ...
    if (!c || c.type === 'action')
      throw invalid('This setting cannot be changed here.');
    if (c.type === 'toggle') {
    ...
    if (c.type === 'number') {
      ...
      if (c.min !== undefined && n < c.min)
        throw invalid(`The minimum is ${c.min}.`);
      if (c.max !== undefined && n > c.max)
        throw invalid(`The maximum is ${c.max}.`);
      return n;
    }
    if (c.type === 'time') {
      if ((value === '' || value === null) && c.nullable) return null;
      if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(String(value)))
        throw invalid('Enter a time like 21:00.');
      return String(value);
    }
    if (c.type === 'select') {
      const v = String(value);
      if (!c.options.some((o) => o.value === v))
        throw invalid('Choose one of the listed options.');
      return v;
    }
    void def;
    return String(value);
  ...
  private async apply(
    ...
    rowKey: string,
    raw: HubValue,
    action: 'setting.changed' | 'setting.reset' | 'setting.restored',
    reason?: string,
  ) {
    const def = await this.findRow(categoryKey, rowKey, ctx);
    if (!this.canEdit(def, ctx))
      throw new ForbiddenException(
        def.requires === 'owner'
          ? 'Only the owner can change this.'
          : 'Your role does not have permission to change this setting.',
      );
    ...
    await this.audit(
      ...
      categoryKey,
      rowKey,
      { value: beforeValue, display: beforeState.value },
      {
        value,
        display: afterState.value,
        label: def.label,
        category: this.category(categoryKey).title,
        reason: reason ?? null,
      },
    );
    return true;
  ...
  async saveChanges(
    ...
    for (const c of changes) {
      if (c.value === undefined)
        throw invalid('A value is needed for every change.');
      if (
        await this.apply(
          ctx,
          c.category,
  ...
  async resetRow(ctx: HubCtx, categoryKey: string, rowKey: string) {
    const def = await this.findRow(categoryKey, rowKey, ctx);
    if (!def.reset)
      throw invalid('This setting has no Noxtill default to return to.');
    await this.apply(
      ctx,
      categoryKey,
      rowKey,
  ...
  private historyEntries(
    ...
    return entries.map((e, i) => {
      ...
      const verb =
        e.action === 'setting.reset'
          ? 'Reset to default'
          : e.action === 'setting.restored'
            ? 'Restored'
            : 'Changed';
      const who = e.actorUserId
        ? (names.get(e.actorUserId) ?? 'Unknown user')
        : 'System';
      return {
        id: e.id,
  ...
  async rowHistory(ctx: HubCtx, categoryKey: string, rowKey: string) {
    const own = this.category(categoryKey).userScoped
      ? { actorUserId: ctx.userId }
      : {};
    ...
    return this.historyEntries(
      entries,
      await this.names(entries.map((e) => e.actorUserId)),
    );
  }
  ...
  async categoryHistory(ctx: HubCtx, categoryKey: string) {
    ...
    const entries = await this.prisma.auditLog.findMany({
      where: {
        businessId: ctx.businessId,
        entity: 'setting',
        entityId: { startsWith: `${categoryKey}.` },
        ...own,
      },
      orderBy: { createdAt: 'desc' },
      take: 40,
    ...
    return entries.map((e) => {
      ...
      return {
        id: e.id,
        rowKey: e.entityId?.split('.').slice(1).join('.') ?? null,
        change: describeAudit(e.action, e.entity, e.after),
        meta: `${e.createdAt.toISOString().slice(0, 16).replace('T', ' ')} UTC · ${e.actorUserId ? (names.get(e.actorUserId) ?? 'Unknown user') : 'System'}${after.display ? '' : ''}`,
      };
    });
  }
  ...
  async restore(
    ...
    if (!entry)
      throw new NotFoundException('That history entry was not found.');
    const after = (entry.after ?? {}) as { value?: HubValue };
    if (after.value === undefined)
      throw invalid('That entry has no value to restore.');
  ...
  async pin(ctx: HubCtx, categoryKey: string, rowKey: string) {
    ...
    const key = {
      businessId_userId_category_rowKey: {
        businessId: ctx.businessId,
        userId: ctx.userId,
    ...
    await this.prisma.settingPin.upsert({
      where: key,
      create: {
        businessId: ctx.businessId,
        userId: ctx.userId,
    ...
    for (const c of this.registry()) {
      for (const g of c.groups) {
        for (const r of g.rows ?? []) {
          if (
            r.requires &&
            !(r.requires === 'owner' ? ctx.isOwner : ctx.can(r.requires))
          )
            continue;
          const hay =
            `${r.label} ${r.description} ${c.label} ${g.title}`.toLowerCase();
  ...
  async runAction(
    ...
    if (actionKey === 'backup-now') {
      if (!ctx.isOwner)
        throw new ForbiddenException('Only the owner can start a backup.');
      const started = await this.backups.startBackup(
        ctx.businessId,
        ctx.userId,
      );
      if (!started) throw invalid('A backup could not be started.');
      await this.audit(ctx, 'setting.action', 'backup', 'backup-now', null, {
        display: 'Backup started',
        label: 'Back up now',
      });
      this.healthCache = null;
      return {
        message: 'Backup started. It appears here as soon as it finishes.',
      };
    }
    if (actionKey === 'backup-download-latest') {
      if (!ctx.isOwner)
        throw new ForbiddenException('Only the owner can download a backup.');
      const latest = await this.backups.latestDownload(ctx.businessId);
      if (!latest)
        throw invalid('There is no finished backup to download yet.');
      await this.audit(
        ctx,
        'setting.action',
        'backup',
        'backup-download',
        null,
        { display: 'Backup downloaded', label: 'Download the latest backup' },
      );
    ...
    if (actionKey === 'sessions-revoke-others') {
      ...
      await this.audit(
        ...
        null,
        {
          display: `${r.count} sessions signed out`,
          label: 'Sign out all other sessions',
        },
      );
      return {
        message: `${r.count} session${r.count === 1 ? '' : 's'} signed out.`,
      };
    }
    if (actionKey.startsWith('session:')) {
      const id = actionKey.slice('session:'.length);
      if (id === ctx.sessionId)
        throw invalid('That is the session you are using right now.');
      await this.sessions.revokeOwn(ctx.userId, id);
      await this.audit(ctx, 'setting.action', 'sessions', `session`, null, {
        display: 'Session signed out',
        label: 'Sign out a session',
      });
      return { message: 'Session signed out.' };
    }
    if (actionKey.startsWith('apikey:')) {
      if (!ctx.can(CAPABILITIES.INTEGRATIONS_MANAGE))
        throw new ForbiddenException('Your role cannot revoke API keys.');
      const id = actionKey.slice('apikey:'.length);
      const key = await this.prisma.apiKey.findFirst({
        where: { id, businessId: ctx.businessId, revokedAt: null },
      });
      if (!key)
        throw new NotFoundException(
          'That API key is already revoked or does not exist.',
        );
      ...
      await this.audit(
        ...
        `apikey`,
        { display: key.name },
        { display: `Revoked key ${key.name}`, label: 'Revoke API key' },
      );
      return {
        message: `Key “${key.name}” revoked. Anything using it stopped working.`,
      };
    }
  ...
  async toggleMatrix(ctx: HubCtx, event: string, channel: string, on: boolean) {
    if (!isNotificationEvent(event) || !isNotificationChannel(channel))
      throw invalid('That notification or channel is not available.');
    await this.deps.notifications.setPreferences(ctx.businessId, {
      userId: ctx.userId,
      preferences: [{ event, channel, enabled: on }],
    });
    return { ok: true as const };
  }
  ...
  async queueSnapshot(): Promise<QueueSnapshot> {
    if (this.queueCache && Date.now() - this.queueCache.at < 30_000)
      return this.queueCache.value;
  ...
  async healthItems(ctx: HubCtx): Promise<HealthItem[]> {
    if (
      this.healthCache &&
      this.healthCache.businessId === ctx.businessId &&
      Date.now() - this.healthCache.at < 20_000
    )
      return this.healthCache.items;
    ...
    const overrides = await this.prisma.roleCapabilityOverride.findMany({
      where: { businessId: ctx.businessId },
    });
    const staffCaps = new Set<string>(
      (overrides.find((o) => o.role === Role.staff)
        ?.capabilities as unknown as string[]) ??
        SYSTEM_ROLE_CAPABILITIES[Role.staff],
    );
    if (staffCaps.has(CAPABILITIES.PROFIT_VIEW))
      push({
        key: 'staff-profit',
        risk: 'Medium',
        tone: 'amber',
        title: 'The Staff role can see profit and margin',
        action: 'Review permissions',
        category: 'team',
      });
    const exposed = SENSITIVE_CAPS.filter(
      (c) => c.key !== CAPABILITIES.PROFIT_VIEW && staffCaps.has(c.key),
    );
    if (exposed.length)
      push({
        key: 'staff-sensitive',
        risk: 'High',
        tone: 'red',
        title: `The Staff role holds ${exposed.length} sensitive permission${exposed.length === 1 ? '' : 's'} (${exposed
          .map((e) => e.label.toLowerCase())
          .slice(0, 2)
          .join(', ')}${exposed.length > 2 ? '…' : ''})`,
        action: 'Review permissions',
        category: 'team',
      });
    ...
    const members = await this.prisma.businessUser.findMany({
      ...
      include: {
        user: { select: { twoFactorEnabled: true, lockedUntil: true } },
      },
    });
    ...
    if (without2fa > 0)
      push({
        ...
        risk: 'Security',
        tone: 'amber',
        title: `${without2fa} owner or manager account${without2fa === 1 ? ' has' : 's have'} no two-factor authentication`,
        action: 'Review security',
        category: 'security',
      });
    const locked = members.filter(
      (m) => m.user.lockedUntil && m.user.lockedUntil > ctx.now,
    ).length;
    if (locked > 0)
      push({
        key: 'locked',
        risk: 'Security',
        tone: 'amber',
        title: `${locked} account${locked === 1 ? ' is' : 's are'} locked after failed sign-ins`,
        action: 'Review security',
        category: 'security',
      });

    const needs = await this.prisma.integration.findMany({
      where: { businessId: ctx.businessId, status: 'needs_attention' },
    });
    for (const i of needs)
      push({
        ...
        risk: 'Communication',
        tone: 'red',
        title: `${i.provider.replace(/_/g, ' ')} needs to be reconnected`,
        action: 'Reconnect',
        category: 'integrations',
      });
    const socialNeeds = await this.prisma.socialAccount.count({
      where: { businessId: ctx.businessId, status: 'needs_attention' },
    });
    if (socialNeeds > 0)
      push({
        ...
        risk: 'Communication',
        tone: 'red',
        title: `${socialNeeds} social channel${socialNeeds === 1 ? ' needs' : 's need'} to be reconnected`,
        action: 'Reconnect',
        category: 'inbox',
      });

    if (b.msgQuota > 0 && b.msgUsed / b.msgQuota >= 0.9)
      push({
        key: 'quota',
        risk: 'Operational',
        tone: 'amber',
        title: `${Math.round((b.msgUsed / b.msgQuota) * 100)}% of the monthly message quota is used`,
        action: 'Review plan',
        category: 'billing',
      });
    if (
      b.trialEndsAt &&
      b.trialEndsAt > ctx.now &&
      !b.stripeSubscriptionId &&
      b.trialEndsAt.getTime() - ctx.now.getTime() < 3 * 86_400_000
    )
      push({
        key: 'trial',
        risk: 'Billing',
        tone: 'amber',
        title: 'Your trial ends in less than 3 days',
        action: 'Choose a plan',
        category: 'billing',
      });
    if (!b.phone || !b.address)
      push({
        key: 'profile',
        risk: 'Missing configuration',
        tone: 'amber',
        title: `Business ${!b.phone && !b.address ? 'phone and address are' : !b.phone ? 'phone is' : 'address is'} not set`,
    ...
    if (!process.env.ANTHROPIC_API_KEY)
      push({
        ...
        risk: 'Operational',
        tone: 'amber',
        title: 'No AI provider is configured, so AI features cannot run',
        action: 'Review AI',
        category: 'ai',
      });

    const failedRuns = await this.prisma.workflowRun.count({
      where: {
        workflow: { businessId: ctx.businessId },
        status: 'failed',
        createdAt: { gte: new Date(ctx.now.getTime() - 86_400_000) },
      },
    });
    if (failedRuns > 0)
      push({
        ...
        risk: 'Operational',
        tone: 'amber',
        title: `${failedRuns} automation run${failedRuns === 1 ? '' : 's'} failed in the last 24 hours`,
        action: 'Review automations',
        category: 'automations',
      });

    for (const c of await configConflicts(this.deps, ctx))
      push({
        key: c.key,
        risk: 'Conflict',
        tone: 'amber',
    ...
    if (resolvePolicies(b).bool('backup.enabled')) {
      ...
      if (s.last?.status === 'failed')
        push({
          ...
          risk: 'Data',
          tone: 'red',
          title: 'The latest backup failed',
        ...
        s.lastReady &&
        ctx.now.getTime() -
          (s.lastReady.readyAt ?? s.lastReady.createdAt).getTime() >
          36 * 3_600_000
      )
        push({
          key: 'backup-overdue',
          ...
          tone: 'amber',
          title:
            'Automatic backup is on but no backup has finished in the last 36 hours',
          action: 'Review backups',
          category: 'backup',
        });
    }
    ...
    if (q && q.configured && !q.reachable)
      push({
        key: 'queue-down',
        risk: 'Operational',
        tone: 'red',
        title:
          'The background job queue is not reachable, so scheduled jobs are not running',
    ...
    if (failedJobs > 0)
      push({
        ...
        risk: 'Operational',
        tone: 'amber',
        title: `${failedJobs} background job${failedJobs === 1 ? '' : 's'} failed`,
        action: 'Open system health',
        category: 'health',
      });
      ...
      rows: [
        ...items.slice(0, 6).map((i) => ({ label: i.risk, value: i.title })),
        {
          ...
          value:
            q && q.configured
              ? q.reachable
                ? `${q.queues.reduce((n, x) => n + x.failed, 0)} failed`
                : 'Queue not reachable'
  ...
  async home(ctx: HubCtx) {
    ...
    const card = (
      ...
      return {
        ...
        icon,
        category,
        status: found.length ? 'Needs attention' : okStatus,
        meta: found.length
          ? found.length === 1
            ? found[0].title
            : `${found.length} items`
    ...
    const recent = await this.prisma.auditLog.findMany({
      where: {
        businessId: ctx.businessId,
        entity: { in: ['setting', 'role_capability_override'] },
      },
      orderBy: { createdAt: 'desc' },
      take: 5,
    });
    ...
    const recentChanges = recent.map((e) => ({
      change:
        e.entity === 'role_capability_override'
          ? `Permissions ${e.action.endsWith('reset') ? 'reset' : 'changed'} for ${((e.after ?? {}) as { role?: string }).role ?? 'a role'}`
          : describeAudit(e.action, e.entity, e.after),
      meta: `${relativeTime(e.createdAt, ctx.now)} · ${e.actorUserId ? (names.get(e.actorUserId) ?? 'Unknown user') : 'System'}`,
      category:
        e.entity === 'role_capability_override'
          ? 'team'
          : (e.entityId?.split('.')[0] ?? 'general'),
    }));
    ...
    const pins = await this.prisma.settingPin.findMany({
      where: {
        businessId: ctx.businessId,
        userId: ctx.userId,
        OR: [{ pinned: true }, { opens: { gte: 2 } }],
      },
      orderBy: [{ pinned: 'desc' }, { opens: 'desc' }],
      take: 6,
    });
    const pinned: {
      label: string;
    ...
    for (const p of pins) {
      ...
      pinned.push({
        label,
        meta: `${cat.label}${p.pinned ? '' : ` · opened ${p.opens} times`}`,
        icon: cat.icon,
        pin: p.pinned ? 'Pinned' : 'Frequent',
        category: p.category,
        rowKey: p.rowKey,
      });
    }
    ...
    return {
      health: [
        ...
        card(
          ...
          ['security'],
          'Healthy',
          'Two-factor and lockout in order',
        ),
        card(
          'Integrations',
          'plug-zap',
        ...
        card(
          ...
          ['ai'],
          'Healthy',
          'AI is limited to reading and drafting',
        ),
        card(
          'System',
          'activity',
        ...
        await this.backupCard(ctx, inCats('backup')),
        {
          ...
          category: 'data',
          status: 'Measured',
          meta: `${counts[0].toLocaleString('en-US')} customers · ${counts[1].toLocaleString('en-US')} orders · ${counts[2].toLocaleString('en-US')} products`,
          tone: 'blue' as HubTone,
        },
        {
          label: 'Notifications',
          icon: 'bell',
          ...
          status: 'Configured',
          meta: `Nightly close at ${b.nightlyCloseTime}`,
          tone: 'green' as HubTone,
        },
      ],
      quickActions: [
  ...
  async ask(ctx: HubCtx, key: string) {
    ...
    if (key === 'profit') {
      const withProfit = (cap: string) =>
        customs.filter((c) =>
          (c.capabilities as unknown as string[]).includes(cap),
        );
      const cRoles = withProfit(CAPABILITIES.PROFIT_VIEW);
      const holders = members.filter((m) =>
        m.customRoleId
          ? cRoles.some((r) => r.id === m.customRoleId)
          : m.role === Role.owner ||
            (m.role === Role.manager &&
              manager.has(CAPABILITIES.PROFIT_VIEW)) ||
            (m.role === Role.staff && staff.has(CAPABILITIES.PROFIT_VIEW)),
      ).length;
      return {
        ...
        rows: [
          ...
          {
            label: 'Manager',
            value: manager.has(CAPABILITIES.PROFIT_VIEW) ? 'Yes' : 'No',
          },
          {
            label: 'Staff',
            value: staff.has(CAPABILITIES.PROFIT_VIEW)
              ? 'Yes · review this'
              : 'No',
            tone: staff.has(CAPABILITIES.PROFIT_VIEW)
              ? ('neg' as const)
              : undefined,
          },
          ...customs.map((c) => ({
            label: `Custom · ${c.name}`,
            value: (c.capabilities as unknown as string[]).includes(
        ...
        bullets: [
          'Profit access exposes cost prices, margins and the tax and P&L reports to everyone in that role',
          'The answer is computed from the current role permissions, including any custom roles',
          'Noxtill does not change any permission on its own',
          'A change takes effect on the person’s next request',
        ],
    ...
    if (key === 'risky') {
      ...
      for (const c of SENSITIVE_CAPS) {
        ...
        if (who === 'owner_manager_staff' || who === 'owner_staff')
          rows.push({
            label: `${c.label}`,
            value: 'Held by Staff · high risk',
            tone: 'neg',
          });
        else if (
          who === 'owner_manager' &&
          [
            CAPABILITIES.EXPORTS_GENERATE,
            CAPABILITIES.CREDIT_WRITE_OFF,
            CAPABILITIES.ROLES_MANAGE,
            CAPABILITIES.BILLING_MANAGE,
            CAPABILITIES.GDPR_MANAGE,
            CAPABILITIES.STAFF_MANAGE,
            CAPABILITIES.PAYROLL_EXPORT,
          ].includes(c.key as never)
        )
          rows.push({
            label: c.label,
            value: 'Held by Manager · normally owner only',
            tone: 'neg',
          });
      }
      ...
      if (no2fa)
        rows.push({
          label: 'Owner or manager without two-factor',
          value: `${no2fa} account${no2fa === 1 ? '' : 's'}`,
          tone: 'neg',
        });
      if (rows.length === 0)
        rows.push({
          label: 'Permission review',
          value: 'Nothing risky found',
          tone: 'pos',
        });
      return {
        ...
        answer:
          rows[0].tone === 'pos'
            ? 'Nothing stands out: Staff holds no sensitive permission and no Manager holds an owner-only one.'
            : `${rows.length} item${rows.length === 1 ? '' : 's'} stand out, ranked by what they expose.`,
        rows,
        bullets: [
          'Checks whether Staff holds any sensitive permission and whether Manager holds owner-only ones',
          'Checks owner and manager accounts for two-factor authentication',
          'Nothing is changed automatically',
          'These are observations, not accusations — your team may have good reasons',
        ],
    ...
    if (key === 'channels') {
      const [ints, socials] = await Promise.all([
      ...
      const rows = [
        ...ints.map((i) => ({
          label: i.provider.replace(/_/g, ' '),
          value:
            i.status === 'connected'
              ? `Connected${i.lastSyncAt ? ` · synced ${relativeTime(i.lastSyncAt, ctx.now)}` : ''}`
              : 'Needs attention',
          tone: i.status === 'connected' ? undefined : ('neg' as const),
        })),
        ...socials.map((s) => ({
          label: s.platform,
          value: s.status === 'connected' ? 'Connected' : 'Needs attention',
          tone: s.status === 'connected' ? undefined : ('neg' as const),
        })),
      ];
      return {
        title: 'What channels are connected?',
        answer: rows.length
          ? `${rows.filter((r) => !r.tone).length} connected and ${rows.filter((r) => r.tone).length} needing attention.`
          : 'No integration or social channel is connected.',
        rows: rows.length
          ? rows
          : [{ label: 'Connected services', value: 'None' }],
        bullets: [
          'Lists integrations and social channels with a recorded connection',
          'A channel needing attention cannot sync until it is reconnected',
          'Customer messages (WhatsApp, SMS, email) use platform-level providers, not a per-business connection',
          'Nothing is shown as connected unless it genuinely is',
        ],
    ...
    if (key === 'recent') {
      ...
      return {
        ...
        answer: recent.length
          ? `${recent.length} recent configuration change${recent.length === 1 ? '' : 's'} are recorded.`
          : 'No configuration change has been recorded yet.',
        rows: recent.length
          ? recent.map((e) => ({
              label: describeAudit(e.action, e.entity, e.after),
              value: `${relativeTime(e.createdAt, ctx.now)} · ${e.actorUserId ? (names.get(e.actorUserId) ?? 'Unknown') : 'System'}`,
            }))
          : [{ label: 'Recent changes', value: 'None' }],
        bullets: [
          'Read directly from the append-only audit log',
          'Only changes made from Settings and role permission edits are included',
          'Each entry keeps the value before and after',
        ],
  ...
  private async backupCard(ctx: HubCtx, problems: HealthItem[]) {
    ...
    if (problems.length > 0)
      return {
        ...
        meta: problems[0].title,
        tone: (problems.some((p) => p.tone === 'red')
          ? 'red'
          : 'amber') as HubTone,
      };
    if (!resolvePolicies(b).bool('backup.enabled'))
      return {
        ...base,
        status: 'Not enabled',
        meta: 'Automatic backup is off',
        tone: 'neutral' as HubTone,
      };
    const s = await backupSummary(this.deps, ctx.businessId, ctx.now);
    return {
      ...base,
      status: 'Healthy',
      meta: s.lastReady
        ? `Last backup ${relativeTime(s.lastReady.readyAt ?? s.lastReady.createdAt, ctx.now)}`
  ...
  async diagnostics(ctx: HubCtx) {
    ...
    const bundle: Record<string, unknown> = {
      ...
      queues: q
        ? {
            ...
            failedJobs: q.queues
              .filter((x) => x.failed > 0)
              .map((x) => ({ queue: x.name, failed: x.failed })),
          }
        : null,
      ...
      ai: {
        providerConfigured: !!process.env.ANTHROPIC_API_KEY,
        callsThisMonth: aiCalls,
      },
      recordCounts: {
        customers: counts[0],
        orders: counts[1],
    ...
    for (const f of DIAGNOSTIC_FIELDS)
      if (!(f.key in bundle))
        throw new Error(`Diagnostic section missing: ${f.key}`);
    return bundle;
  }
}
