...
export class SerpRankService {
  ...
  async fetchRank(keyword: string, businessName: string): Promise<number | null> {
    ...
    try {
      ...
      const message = axios.isAxiosError(error) ? error.response?.data : (error as Error).message;
