...
describe('signed-token.util (BE-082)', () => {
  ...
  it('round-trips a signed payload', () => {
    const token = signPayload<TestPayload>(
      { businessId: 'biz-1', provider: 'gmb' },
      secret,
    );
  ...
  it('rejects a tampered payload even with a valid-looking signature', () => {
    ...
    const tamperedJson = Buffer.from(
      JSON.stringify({ businessId: 'biz-2', provider: 'gmb' }),
    ).toString('base64url');
    expect(
      verifyPayload<TestPayload>(`${tamperedJson}.${signature}`, secret),
    ).toBeNull();
