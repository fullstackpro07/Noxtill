...
describe('GmbConnector (BE-084, via GoogleOAuth2Connector)', () => {
  ...
  it('pushListing() rejects with a clear error when no locationId has been selected (UPD-BE-041)', async () => {
    await expect(
      connector.pushListing(
        { accessToken: 'fake-token' },
        { name: 'Real Biz', categories: [], hours: {} },
        {},
      ),
  ...
  it('pushListing() PATCHes the real Business Information API once a locationId is set', async () => {
    mockedAxios.patch.mockResolvedValue({ data: { name: 'locations/123' } });
    ...
    expect(mockedAxios.patch).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ title: 'Real Biz' }),
      expect.objectContaining({
        headers: { Authorization: 'Bearer fake-token' },
      }),
    );
