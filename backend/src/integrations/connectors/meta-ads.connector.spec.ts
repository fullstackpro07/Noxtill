...
describe('MetaAdsConnector (BE-087)', () => {
  ...
  it('builds a real facebook.com authorize URL with the ads_management scope', () => {
    ...
    expect(url.searchParams.get('redirect_uri')).toBe('http://localhost:5000/api/v1/integrations/meta_ads/callback');
  ...
  it('exchanges the code via a GET to the real Graph API token endpoint (not POST, per Meta)', async () => {
    mockedAxios.get.mockResolvedValue({ data: { access_token: 'meta-token', token_type: 'bearer', expires_in: 5184000 } });
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ params: expect.objectContaining({ code: 'some-code' }) }),
    );
  });

  it("refreshToken() performs Meta's real fb_exchange_token long-lived-token exchange, not a standard refresh grant", async () => {
    mockedAxios.get.mockResolvedValue({ data: { access_token: 'long-lived-token', token_type: 'bearer' } });
    await connector.refreshToken({ accessToken: 'short-lived-token' });
    // eslint-disable-next-line @typescript-eslint/unbound-method
    expect(mockedAxios.get).toHaveBeenCalledWith(
      'https://graph.facebook.com/v19.0/oauth/access_token',
      expect.objectContaining({
        params: expect.objectContaining({ grant_type: 'fb_exchange_token', fb_exchange_token: 'short-lived-token' }),
      }),
    );
  });
});
