...
describe('IntegrationsService (BE-082)', () => {
  ...
  const emailConnector = {
    ...
    handleCallback: jest.fn().mockResolvedValue({ accessToken: '' }),
    refreshToken: jest.fn(),
    sync: jest.fn(),
    disconnect: jest.fn().mockResolvedValue(undefined),
  };
  ...
  it('connect() on the non-OAuth email provider connects immediately, no redirect, and stores real tokens', async () => {
    ...
    // Fix for a real gap: connect() used to skip storing tokens for non-OAuth providers, so
    // getTokens() always came back null even after a successful "connect".
    expect(row?.tokens).not.toBeNull();
    const tokens = await service.getTokens(
      businessId,
      IntegrationProvider.email,
    );
    expect(tokens).toEqual({ accessToken: '' });
  });
