...
function signPayload(payload, secret) {
    ...
    const signature = (0, crypto_1.createHmac)('sha256', secret).update(json).digest('base64url');
...
function verifyPayload(token, secret) {
    ...
    if (!json || !signature)
        return null;
    const expected = (0, crypto_1.createHmac)('sha256', secret).update(json).digest('base64url');
