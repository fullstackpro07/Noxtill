...
/** "New Quotation" drawer — real product-based line items (the design's free-text "Event styling
 * package" custom line doesn't exist as a backend concept; quotations use the same real
 * product/qty/price-override items every other order type uses). */
export function QuotationFormDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [terms, setTerms] = useState("50% advance, balance on delivery. Prices valid until the date above.");
  ...
  const mutation = useMutation({
    ...
    onSuccess: (quote) => {
      ...
      toast.success(`Quotation ${quote.quoteNo} saved as draft.`);
      reset();
      onClose();
    },
    onError: (err) => toast.error(err instanceof ApiError ? err.message : "Couldn't save this quotation — please try again."),
  });

  return (
    <SlideDrawer open={open} onClose={onClose} title="Quotation Builder">
      <div className="flex flex-col gap-[13px]">
        ...
        <div className="rounded-[12px] p-3" style={{ border: "1px solid var(--app-border)" }}>
          <div className="mb-2 text-[11px] font-bold" style={{ color: "var(--app-text-disabled)" }}>LINE ITEMS</div>
          {lines.length > 0 && (
              ...
              {lines.map((l) => (
                <div key={l.productId} className="flex items-center gap-2.5">
                  ...
                  <input
                    ...
                    className="w-[62px] rounded-[9px] p-2 text-center text-[12.5px] font-bold"
                    style={{ border: "1px solid var(--app-border)" }}
                  />
                  <span className="w-[86px] text-end text-[12.5px] font-extrabold" style={{ color: "var(--app-text)" }}>{formatCurrency(l.price * l.qty, session.business.currency)}</span>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <label className="block">
            <span style={fieldLabel}>VALID UNTIL</span>
            <input type="date" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} style={{ ...fieldStyle, minHeight: 46 }} />
          </label>
        </div>

        <label className="block">
          <span style={fieldLabel}>TERMS</span>
          <textarea value={terms} onChange={(e) => setTerms(e.target.value)} rows={3} style={{ ...fieldStyle, fontFamily: "inherit", fontWeight: 500, resize: "vertical" }} />
        </label>
